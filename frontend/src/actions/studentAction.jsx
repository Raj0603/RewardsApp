import {
  ...
  export const studentLogin = (email, password) => async (dispatch) => {
    try {
      ...
      const data = await axios.post(`/api/v1/slogin`, { email, password }, config);
  ...
  export const studentRegister = (studentData) => async (dispatch) => {
    try {
      ...
      const { data } = await axios.post(`/api/v1/sregister`, studentData, config);
  ...
  export const loadStudent = () => async (dispatch) => {
    try {
      ...
      const { data } = await axios.get(`/api/v1/sme`);
  ...
  export const logoutStudent = () => async (dispatch) => {
    try {
      await axios.get(`/api/v1/slogout`);
  ...
  export const updateStudentProfile = (studentData) => async (dispatch) => {
    try {
      ...
      const { data } = await axios.put(`/api/v1/sme/update`, studentData, config);
  ...
  export const forgotPassword = (email) => async (dispatch) => {
    try {
      ...
      const config = { headers: { "Content-Type": "application/json" } };
      const {data} = await axios.post(`/api/v1/spassword/forgot`, email, config);
  ...
  export const getAllStudents = () => async (dispatch) => {
    try {
      ...
      const { data } = await axios.get(`/api/v1/sadmin/students`);
  ...
  export const getAllOwners = () => async (dispatch) => {
    try {
      ...
      const { data } = await axios.get(`/api/v1/sadmin/owner`);
    ...
    export const deleteOwner = (id) => async (dispatch) => {
      try {
        ...
        const { data } = await axios.delete(`/api/v1/sadmin/owner/${id}`);
  ...
  export const getStudentDetails = (id) => async (dispatch) => {
    try {
      ...
      const { data } = await axios.get(`/api/v1/sadmin/student/${id}`);
